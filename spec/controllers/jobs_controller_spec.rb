...
describe JobsController do
  ...
  describe 'POST /sites/{{site_id}}/jobs' do
    ...
    it 'should return 400 if an advance reservation is not valid (if not a String)' do
      authenticate_as('crohr')
      payload = @valid_job_attributes
      payload['reservation'] = 1720
      post :create, params: { site_id: 'rennes', format: :json }, body: payload.to_json, as: :json
      expect(response.status).to eq 400
      expect(response.body).to eq "The job you are trying to submit is not valid: `reservation` should be a String."
    end
