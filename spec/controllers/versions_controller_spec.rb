...
describe VersionsController do
  ...
  describe "GET {{resource}}/versions" do
    it "should get the list of versions" do
      ...
      expect(json["links"].map{|l| l["rel"]}.sort).to eq(["parent", "self"])
      expect(json["items"][0].keys.sort).to eq(["author", "date", "links", "message", "type", "uid"])
      expect(json["items"][0]["links"].map{|l| l["rel"]}.sort).to eq(["parent", "self"])
    end
  ...
  describe "GET {{resource}}/versions/{{version_id}}" do
    ...
    it "should return the version" do
      ...
      expect(json.keys.sort).to eq(["author", "date", "links", "message", "type", "uid"])
