...
describe DeploymentsController do
  ...
  describe "GET /sites/{{site_id}}/deployments/{{id}}" do
    it "should return 404 if the deployment does not exist" do
      ...
      expect(response.body).to match %r{Couldn't find Grid5000::Deployment with ID=doesnotexist}
    end
    it "should return 200 and the deployment" do
      ...
      expect(json.keys.sort).to eq(["created_at", "disable_bootloader_install", "disable_disk_partitioning", "environment", "ignore_nodes_deploying", "links", "nodes", "site_uid", "status", "uid", "updated_at", "user_uid"])
    end
