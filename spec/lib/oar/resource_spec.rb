...
describe OAR::Resource do
  ...
  describe 'status for multiple resource types' do
    ...
    it 'should return status for all requested resource types' do
      expect(OAR::Resource.status({ types: %w[node disk] }).keys.sort).to eq %w[nodes disks].sort
    end
  end
  ...
  describe 'status for nodes' do
    it 'should return the status of all the default resources' do
      ...
      OAR::Resource.all.each do |resource|
    ...
    it 'should return the status only for the resources belonging to the given clusters' do
      expect(OAR::Resource.status(clusters: %w[paradent paramount])['nodes'].keys
               .map { |n| n.split('-')[0] }.uniq.sort).to eq %w[paradent paramount]
    end
    ...
    describe 'standby state' do
      ...
      it 'should return a standby node' do
        expect(OAR::Resource.status['nodes'].select do |_node, status|
                 status[:hard] == 'standby'
               end.map { |(node, _status)| node }).to eq ['paramount-2.rennes.grid5000.fr']
      end
    end
  end
  ...
  describe 'Status of disks' do
    ...
    it 'should return all disks with status busy' do
      expect(OAR::Resource.status(types: ['disk'])['disks'].select do |_disk, status|
               status[:soft] == 'busy'
             end.map { |disk, _status| disk }.sort).to eq ['sdb.parasilo-1.rennes.grid5000.fr', 'sdc.parasilo-1.rennes.grid5000.fr'].sort
    end
