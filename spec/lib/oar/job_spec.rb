...
describe OAR::Job do
  ...
  it "should fetch the list of active jobs" do
    expect(OAR::Job.active.map(&:uid)).to eq([374173, 374179, 374180, 374185, 374186, 374190, 374191, 374192, 374193, 374194, 374195, 374196, 374197, 374198, 374199])
  ...
  it "should fetch the predicted start time" do
    expect(OAR::Job.find(374191).gantt.start_time).to eq(1294395995)
  end
  
  # abasu : updated job from "last" to "first" (which remains same) -- 2015.04.07
  it "should fetch the job events" do
    expect(OAR::Job.active.first.events.map(&:type)).to eq(["FRAG_JOB_REQUEST", "SEND_KILL_JOB", "SWITCH_INTO_ERROR_STATE"])
  end
